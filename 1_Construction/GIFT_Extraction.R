...
unique_geo_entities = unique(checklists[[1]]$entity_ID)

save(unique_geo_entities, file = "GIFT_UniqueEntities.RData")


save(checklists, env, gift_traits, gift_traits_tax, refs_traits_3.6.1, 
    file = "GIFT_LatGrad_extended.RData")

traits_list <- GIFT_traits_meta(
  api = "https://gift.uni-goettingen.de/api/extended/",
  GIFT_version = "3.0"
)

gift_extra_traits <- GIFT_traits(trait_IDs = c("1.1.1", "2.1.1", "1.2.1", "1.2.2"), GIFT_version="3.0", api = api)
saveRDS(gift_extra_traits, "data/GIFT_extra_traits_extended.Rds")

write.csv(traits_list, "data/tmp_traits_list.csv")
